...
**Blender Import Algorithm (Roof Slope Handling):**

The roof geometry in `Ifc_Object_Library.db` is already sloped (Z variance ≈0.26m). Import code must use `end_point` to position and orient the slope.

```python
# blender_lod300_import.py:create_roof_from_geometry()
def create_roof_from_geometry(name, geometry_data, position, end_point, dimensions):
    """Position sloped roof from eave to ridge"""

    # 1. Create mesh from library (already sloped)
    mesh = create_mesh_from_geometry(name, geometry_data, base_rotation)

    # 2. Calculate placement and rotation
    start = Vector(position)       # Eave point: [5.6, 8.5, 3.0]
    end = Vector(end_point)         # Ridge point: [5.6, 4.25, 4.98]
    direction = end - start         # Vector from eave to ridge
    mid_point = (start + end) / 2   # Place at midpoint

    # 3. Calculate pitch angle (tilt)
    horizontal_dist = sqrt(direction.x² + direction.y²)  # 4.25m
    vertical_rise = direction.z                           # 1.98m
    pitch_angle = atan2(vertical_rise, horizontal_dist)   # 25° = 0.436 rad

    # 4. Calculate compass direction (yaw)
    yaw_angle = atan2(direction.y, direction.x)  # North: -90°, South: +90°

    # 5. Apply transformations
    obj.location = mid_point
    obj.rotation_euler = Euler((pitch_angle, 0, yaw_angle), 'XYZ')
    obj.scale = (width_scale, slope_length_scale, 1.0)
```

**Critical Implementation Details:**
- Roof tile geometry has Z variance (not flat)
- `end_point` defines ridge position
- Pitch angle calculated from rise/run
- Yaw angle orients roof north/south
- **Without this handling, roof appears flat!**

**Gutter Placement Algorithm:**

Gutters placed at building perimeter at eave height.

```python
# core/extraction_engine.py:1894-1929
# North gutter: Y=building_length (8.5m) - NORTH wall
{
    'name': 'gutter_north',
    'position': [building_width/2, building_length, building_height],  # [5.6, 8.5, 3.0]
    'length': building_width,      # 11.2m
    'orientation': 0.0              # Along X-axis
}

# South gutter: Y=0 - SOUTH wall
{
    'name': 'gutter_south',
    'position': [building_width/2, 0, building_height],  # [5.6, 0.0, 3.0]
    'length': building_width,      # 11.2m
    'orientation': 0.0              # Along X-axis
}

# East gutter: X=0 - WEST wall
{
    'name': 'gutter_east',
    'position': [0, building_length/2, building_height],  # [0.0, 4.25, 3.0]
    'length': building_length,     # 8.5m
    'orientation': 90.0             # Along Y-axis
}

# West gutter: X=building_width (11.2m) - EAST wall
{
    'name': 'gutter_west',
    'position': [building_width, building_length/2, building_height],  # [11.2, 4.25, 3.0]
    'length': building_length,     # 8.5m
    'orientation': 90.0             # Along Y-axis
}
```

**Gutter Naming Convention:**
- `gutter_north` = max Y (8.5m)
- `gutter_south` = min Y (0.0m)
- `gutter_east` = min X (0.0m)
- `gutter_west` = max X (11.2m)
