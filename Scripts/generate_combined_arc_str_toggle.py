...
# Common bounds after alignment (both should overlap around 2k-62k range)
COMBINED_BOUNDS = {'min_x': 0, 'max_x': 65000, 'min_y': -90000, 'max_y': -40000}
...
def extract_arc_layers(dxf_path: str) -> Dict:
    ...
    for entity in doc.modelspace():
        ...
        if entity.dxftype() == 'CIRCLE':
            c = entity.dxf.center
            x_aligned = c.x + ARC_OFFSET['x']
            y_aligned = c.y + ARC_OFFSET['y']
            if point_in_bounds(x_aligned, y_aligned, COMBINED_BOUNDS):
                grouped[group].append({
                    'type': 'circle',
                    'cx': x_aligned, 'cy': y_aligned,
                    'r': entity.dxf.radius
                })

        elif entity.dxftype() == 'LINE':
            s, e = entity.dxf.start, entity.dxf.end
            x1, y1 = s.x + ARC_OFFSET['x'], s.y + ARC_OFFSET['y']
            x2, y2 = e.x + ARC_OFFSET['x'], e.y + ARC_OFFSET['y']
            if (point_in_bounds(x1, y1, COMBINED_BOUNDS) or
                point_in_bounds(x2, y2, COMBINED_BOUNDS)):
                grouped[group].append({
                    'type': 'line',
            ...
            points = [(p[0] + ARC_OFFSET['x'], p[1] + ARC_OFFSET['y'])
