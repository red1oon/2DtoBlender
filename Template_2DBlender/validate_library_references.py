...
def validate_library_references(template_path, database_path):
    ...
    if 'objects' not in tables:
        print(f"❌ ERROR: Database does not have 'objects' table")
        print(f"Available tables: {tables}")
        conn.close()
        return {
            'status': 'error',
            'error': 'objects table not found',
    ...
    print(f"✅ 'objects' table columns: {columns}")
    print()
    ...
    for obj_type in sorted(object_types):
        try:
            # Query database for this object_type
            query = f"SELECT COUNT(*) FROM objects WHERE {lookup_column} = ?"
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("      ~/Documents/bonsai/8_IFC/Ifc_Object_Library.db")
        sys.exit(1)
